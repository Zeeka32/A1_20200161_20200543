...
Matrix<T> Matrix <T>:: operator * (const Matrix& rhs) {
    if (nColumn_ == rhs.nRow_) {
        ...
        Matrix <T> result (0, 0);
        cout << "failed operation, columns of M1 must be equal to that of the rows of M2" << endl;
        return result;
    }
}

template <typename T>
istream& operator >> (istream& in,typename T:: Matrix& rhs) {
...
template <typename T>
ostream& operator << (ostream& out,const typename T:: Matrix& rhs) {
    for (int i = 0; i < rhs.nRow_; i++) {
        for (int j = 0; j < rhs.nColumn_; j++) {
            cout << rhs.matrix_ [i][j] << '\t';
        }
        cout << endl;
    }
    return out;
