...
Matrix<T> Matrix <T>:: operator + (const Matrix& rhs) {
    if (nRow_ == rhs.nRow_ && nColumn_ == rhs.nColumn_) {
        ...
        Matrix <T> result (0, 0);
        std::cout << "failed operation, dimensions must be of equal size." << std::endl;
        return result;
    }
}
...
Matrix<T> Matrix <T>:: operator * (const Matrix& rhs) {
    if (nColumn_ == rhs.nRow_) {
        ...
        Matrix <T> result (0, 0);
        std::cout << "failed operation, columns of M1 must be equal to that of the rows of M2" << std::endl;
        return result;
    }
}
...
template <typename T2>
ostream& operator << (ostream& out, const Matrix<T2>& rhs) {
...
template <typename T2>
istream& operator >> (istream& in, Matrix<T2> &rhs) {
    for (int i = 0; i < rhs.nRow_; i++) {
        for (int j = 0; j < rhs.nColumn_; j++) {
            in >> rhs.matrix_[i][j];
        }
