...
public:
    void runCalculator (void) {
        ...
        while (true)
        {
            ...
            //check for input validation. catch stoi exception, or end program.
            while(true){
                try{
                cout << ">>> ";
                cin >> lhs >> insertedOperator >> rhs;
                    ...
                    if(error == "off" || error == "OFF")
                        return;
                    cout << error << "\n\n";
                    //ignores the rest of the line if the users enters wrong input and clears the input stream.
